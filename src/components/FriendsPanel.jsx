...
export default function FriendsPanel({ variant = "embedded" }) {
  ...
  const openDmWithFriend = (otherUserId) => {
    const cid = dmConversationId(authUser?._id, otherUserId);
    setSelectedConversation({
      conversationId: cid,
      type: "DM",
      otherUserId,
  ...
  const openFriendProfile = (otherUserId) => {
    navigate(`/profile/${encodeURIComponent(otherUserId)}`);
  };
  ...
  const removeSelectedFriend = async (otherUserId) => {
            ...
            <div className="space-y-2">
              {friends.map((f) => {
                ...
                return (
                  <div
                    key={f.otherUserId}
                    className="discord-list-item justify-between rounded-lg border border-white/10 bg-black/10 transition hover:border-white/20 hover:bg-white/5"
                  >
                    <button
                      type="button"
                      className="flex min-w-0 flex-1 items-center gap-3 text-left"
                      onClick={() => openFriendProfile(f.otherUserId)}
                      title="Xem hồ sơ"
                    >
                      <img
                        src={avatar}
                    ...
                    <div className="flex shrink-0 gap-2">
                      <button
                        ...
                        onClick={() => openDmWithFriend(f.otherUserId)}
                      >
                        Chat
                      </button>
                      <button
                        type="button"
                        className="btn btn-xs rounded-md border-0 bg-red-500/15 text-red-200 hover:bg-red-500/25"
                        onClick={() => removeSelectedFriend(f.otherUserId)}
                      >
                        Hủy kết bạn
                      </button>
                    </div>
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-(--discord-chat)">
